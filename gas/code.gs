...
function searchRepeater(name, email) {
  ...
  // 列インデックスを特定
  const idx = {
    name: headers.indexOf('お名前'),
    email: headers.indexOf('メールアドレス'),
    furigana: headers.indexOf('ふりがな'),
    phone: headers.indexOf('電話番号'),
    zip: headers.indexOf('郵便番号'),
    address: headers.indexOf('ご住所'),
    ...
    photoUrl: headers.indexOf('プロフィール画像URL'),
    // SNSは1つのカラムにまとめられている
    snsLinks: headers.indexOf('SNSリンク')
  };
...
function doPost(e) {
  ...
  try {
    ...
    // スプレッドシート保存 (二重保存)
    saveToSpreadsheet(data, calculationResult, params.currentSpreadsheetId, params.databaseSpreadsheetId);
...
// スプレッドシート保存（二重保存対応）
function saveToSpreadsheet(data, calculationResult, currentSsId, databaseSsId) {
  // 1. 今回のイベント用シートへ保存 (ID指定があればそれを使う、なければConfigのデフォルト)
  const targetId = currentSsId || CONFIG.SPREADSHEET_ID;
  saveToSingleSpreadsheet(targetId, data, calculationResult);
  
  // 2. マスターデータベースへ保存 (ID指定があれば、かつ上記と異なれば保存)
  // Configにマスター用IDがあればそれも使う
  const masterId = databaseSsId || CONFIG.SPREADSHEET_ID; // 一旦デフォルトと同じにしておく、運用で切り替える
  
  if (masterId && masterId !== targetId) {
     saveToSingleSpreadsheet(masterId, data, calculationResult);
  }
}

// 単一スプレッドシートへの保存処理
function saveToSingleSpreadsheet(spreadsheetId, data, calculationResult) {
  try {
